#!/usr/bin/env bash
# Backup <-> deploy sync for config files not managed via home-manager modules.
# Logic per path:
#   - target exists          -> backup (copy target into repo)
#   - target missing, repo has it -> deploy (copy repo into target)
#   - neither exists         -> skip
set -euo pipefail
